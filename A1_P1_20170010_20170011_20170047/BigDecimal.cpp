...
using namespace std;
BigDecimal s3;
BigDecimal::BigDecimal(string numm)
{
    num=numm;
}
BigDecimal::BigDecimal(int numm)
{
...
void string_absolute(string &absolute)//to remove negative
{
    ...
    for(unsigned int i=1; i<absolute.length(); i++)
    {
...
BigDecimal BigDecimal:: operator+ (BigDecimal s2)
{
    ...
    {
        for(unsigned int tryy=0; tryy<(s2.num.length()-num.length()); tryy++)
        {
    ...
    {
        for(unsigned int tryy=0; tryy<(num.length()-s2.num.length()); tryy++)
        {
            num_for_append.append("0");//to ad 0 in left of num
        }
...
BigDecimal BigDecimal:: operator- (BigDecimal s2)
{
    ...
    if (s2.num.length()>num.length())
    {
        for(unsigned int tryy=0; tryy<(s2.num.length()-num.length()); tryy++)//to ad 0 in left of num
        {
